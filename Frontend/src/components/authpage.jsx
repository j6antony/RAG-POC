...
import { submitPassword, signup as createAccount } from '../services/api';
  ...
  return (
    <main className="auth-shell">
      <section className="auth-story" aria-label="About Document chat">
        <a className="auth-brand" href="#" onClick={(event) => { event.preventDefault(); setMode('login'); setError(''); setShowPassword(false); }}><span className="brand-mark" aria-hidden="true">D</span>Document chat</a>
        <div className="auth-story-content">
          ...
          <div className="auth-preview" aria-hidden="true">
            <div className="preview-file"><span>▤</span> Employee handbook <span>PDF</span></div>
      ...
      <section className="auth-form-panel" aria-labelledby="auth-title">
        <div className="auth-form-container">
          ...
          <form key={mode} onSubmit={submit} className="auth-form" onChange={() => setError('')}>
            ...
            <button className="auth-submit" type="submit">{signup ? 'Create account' : 'Log in'}<span aria-hidden="true">→</span></button>
          </form>
          <p className="auth-switch">{signup ? 'Already have an account?' : 'New to Document chat?'} <button type="button" onClick={switchMode}>{signup ? 'Log in' : 'Sign up'}</button></p>
          <p className="auth-demo-note">Frontend preview · Use any email and password to explore. No account is created, and your password isn’t saved or sent.</p>
        </div>
      </section>
    </main>
  );
